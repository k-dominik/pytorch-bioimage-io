...
def simple_train(config, n_iterations=500, batch_size=4, num_workers=2,
    ...
    model_class = config.model.callable_
    ...
    reader_class = train_config.reader.callable_
    reader_kwargs = train_config.reader.kwargs
    reader = reader_class(**reader_kwargs)

    sampler_class = train_config.sampler.callable_
    ...
    preprocess = [conf.callable_(**conf.kwargs) for conf in preprocess_config]

    loss_config = train_config.loss
    loss = [conf.callable_(**conf.kwargs) for conf in loss_config]

    optimizer_class = train_config.optimizer.callable_
